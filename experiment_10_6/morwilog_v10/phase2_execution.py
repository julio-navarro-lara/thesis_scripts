...
def increment_decrement_ph(aasg_set,branch,aasg_pos,verdict,p_delta_ph_0,p_omega,p_initial_ph,p_minimum_ph):
    ...
    for i in range(0,number_events):
        ...
        if i in dict_changes_to_made:
            for result in dict_changes_to_made[i]:
                ...
                sequence = result["sequence"]
                aasg_set = increment_decrement_ph(aasg_set,branch,aasg_pos,expert_evaluation(sequence),dict_var["p_delta_ph_0"],dict_var["p_omega"],dict_var["p_initial_ph"],dict_var["p_minimum_ph"])
