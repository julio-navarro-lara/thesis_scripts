    ...
    for i in range(0,number_events):
        ...
        if id_event in dict_changes_to_made:
            for result in dict_changes_to_made[id_event]:
                ...
                verdict_expert = expert_evaluation(sequence)
                aasg_set[aasg_pos] = update_aasg_set_aasg(aasg_set[aasg_pos],branch,verdict_expert,dict_var["p_learning_rate"])

                if verdict_expert:
                    true_positives += len(sequence)
                else:
                    false_positives += len(sequence)
