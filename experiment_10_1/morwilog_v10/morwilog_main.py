    ...
    for arc_set in aasg["arcs"]:
        if arc_set["start"] == node1:
            ...
            for child in arc_set["children"]:
                ...
                if child["id"] == node2:
                    chosen_ph = ph
            return float(chosen_ph)/float(sum_ph)
...
def main(dict_var,file_logs,file_aasg):
    ...
    start = time.time()
    result = morwihill(e_clas,aasg_set,dict_var)
    end = time.time()
    ...
    #["initial_ph","minimum_ph","delta_ph_0","Tmax","rho","omega","P","N","TP","FP","Alerts","B[0,1,3]","B[0,1,4]","B[0,2,5]","B[0,2,6]"]
    return [dict_var["p_initial_ph"],
            dict_var["p_minimum_ph"],
            dict_var["p_delta_ph_0"],
            dict_var["p_t_max"],
            dict_var["p_evap_rate"],
            dict_var["p_omega"],
            result[1],
            result[2],
            result[3],
