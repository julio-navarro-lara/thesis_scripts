...
#We add the optional nodes and counters
import sys
import json
...
def extract_relative_condition(split_condition,function_name):
    cond = {}
    cond["function"] = split_condition[0]
    attC = split_condition[1].split('$_')
    attP = split_condition[2].split('$_')
    ...
    if len(attP) == 1 and function_name != "SIM_COM":
        cond["attP"] = attP[0]
    else:
        cond["attP"] = attP

    if len(attC) == 1 and function_name != "SIM_COM":
        cond["attC"] = attC[0]
    else:
        cond["attC"] = attC
...
def extract_e_star(e_star_cy):
    ...
    for condition in e_star_cy:
        ...
        cond = {}
        if "SIM" in function:
            cond = extract_relative_condition(split_condition,function)
...
def extract_nodes(set_nodes):
    ...
    for node in set_nodes:
        ...
        new_node = {"id":node["data"]["id"],"e_star":e_star}

        if "optional" in node["data"]:

            #If it is empty, it is No by default
            if not node["data"]["optional"]:
                new_node["optional"] = False
            else:
                new_node["optional"] = node["data"]["optional"]

        if "counter" in node["data"]:

            #If it is empty, it is 1 by default
            if not node["data"]["counter"]:
                new_node["counter"] = 1
            else:
                new_node["counter"] = node["data"]["counter"]

        nodes_aasg.append(new_node)
